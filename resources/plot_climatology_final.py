...
class GeospatialBoundaryManager:
    ...
    def load_iran_provinces(self) -> Optional[gpd.GeoDataFrame]:
        """
        Attempts to load administrative province boundaries of Iran locally or online.
        """
        local_path = os.path.join(self.base_dir, 'inputs', 'iran_provinces.geojson')
        if os.path.exists(local_path):
            try:
                print(f"[BOUNDARIES] Loading local Iran provinces: {local_path}")
                return gpd.read_file(local_path)
            except Exception as e:
                print(f"[BOUNDARIES WARNING] Failed to load local provinces geojson: {e}")

        # Fallback to online repo
        online_url = "https://raw.githubusercontent.com/mrunderline/iran-geojson/master/ir_states_boundaries_coordinates.geojson"
        try:
            print(f"[BOUNDARIES] Fetching Iran provinces online...")
            return gpd.read_file(online_url)
        except Exception as e:
            print(f"[BOUNDARIES WARNING] Failed to fetch online Iran provinces boundaries: {e}")
            return None
